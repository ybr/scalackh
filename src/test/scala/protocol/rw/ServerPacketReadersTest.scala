...
object ServerPacketReadersTest extends SimpleTestSuite {
  test("Consumed BlockInfo(true,10)") {
    ...
    val Consumed(result) = ServerPacketReaders.blockInfoReader.read(in)
  ...
  test("NotEnough field bucketNum BlockInfo(false,-1)") {
    ...
    val result = ServerPacketReaders.blockInfoReader.read(in)
